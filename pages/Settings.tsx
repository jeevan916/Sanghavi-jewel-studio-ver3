...
import { Save, Plus, Trash2, Lock, Unlock, Settings as SettingsIcon, X, MessageCircle, Loader2 } from 'lucide-react';
    ...
    <div className="max-w-4xl mx-auto p-4 md:p-8 pb-24">
      <header className="mb-8 flex items-center justify-between">
        <div>
            ...
            <p className="text-stone-500">Manage suppliers, categories, and system config.</p>
        </div>
        <button 
            onClick={handleSave}
            className="px-6 py-2 bg-stone-900 text-white rounded-lg flex items-center gap-2 hover:bg-stone-800 transition shadow-lg"
        >
            ...
            <div className="bg-white p-6 rounded-xl border border-stone-100 shadow-sm">
                ...
                <div className="flex gap-2 mb-6">
                    <input 
                        ...
                        className="flex-1 p-2 border border-stone-200 rounded-lg"
                    />
                    <button onClick={addSupplier} className="bg-gold-600 text-white px-4 rounded-lg"><Plus /></button>
                </div>
                    ...
                    {config.suppliers.map(s => (
                        <div key={s.id} className="flex items-center justify-between p-3 bg-stone-50 rounded-lg border border-stone-100">
                             ...
                             <div className="flex items-center gap-2">
                                 <button onClick={() => toggleSupplierPrivacy(s.id)} className="p-2 text-stone-400 hover:text-stone-600">
                                     {s.isPrivate ? <Lock size={16}/> : <Unlock size={16}/>}
                                 </button>
                                 <button onClick={() => deleteSupplier(s.id)} className="p-2 text-stone-400 hover:text-red-500">
                                     <Trash2 size={16}/>
                                 </button>
                             </div>
                        </div>
            ...
            <div className="bg-white p-6 rounded-xl border border-stone-100 shadow-sm">
                ...
                <div className="flex gap-2 mb-6">
                    <input 
                        ...
                        className="flex-1 p-2 border border-stone-200 rounded-lg"
                    />
                    <button onClick={addCategory} className="bg-gold-600 text-white px-4 rounded-lg"><Plus /></button>
                </div>
                        ...
                        <div key={c.id} className="border border-stone-200 rounded-xl overflow-hidden">
                            <div className="bg-stone-50 p-3 flex justify-between items-center border-b border-stone-200">
                                ...
                                <div className="flex gap-2">
                                     <button onClick={() => toggleCategoryPrivacy(c.id)} className="text-stone-400 hover:text-stone-600 text-xs uppercase font-bold">
                            ...
                            <div className="p-4 bg-white">
                                ...
                                <div className="flex gap-2">
                                    <input 
                                        ...
                                        className="flex-1 text-sm p-1.5 border border-stone-200 rounded"
                                    />
                                    <button onClick={() => addSubCategory(c.id)} className="text-xs bg-stone-800 text-white px-3 rounded">Add</button>
                                </div>
                            </div>
                        </div>
                    ))}
