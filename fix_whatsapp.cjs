...
const syncIndex = content.indexOf("    router.post('/templates/:id/sync'");
const logsIndex = content.indexOf("    // 8. GET LOGS");

if (syncIndex === -1 || logsIndex === -1) {
    console.log("Could not find boundaries");
    process.exit(1);
}

const replacement = `    // 6. SYNC TEMPLATE (STRICT FETCH FROM META)
...
const newContent = content.substring(0, syncIndex) + replacement + content.substring(logsIndex);
