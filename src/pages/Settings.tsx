        ...
        <div className="space-y-6 animate-in fade-in duration-300">
             ...
             <div className="bg-white p-6 rounded-xl border border-stone-100 shadow-sm">
                ...
                <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                            <label className="block text-xs font-bold text-stone-400 uppercase tracking-widest mb-2">WhatsApp Business Account ID (WABA ID)</label>
                            <input type="text" value={config.whatsappWabaId || ''} onChange={e => setConfig({...config, whatsappWabaId: e.target.value})} placeholder="Optional - Bypasses dynamic resolution if set" className="w-full p-2 border border-stone-200 rounded-lg text-sm text-stone-900" />
                        </div>
                        <div>
                        ...
                        <div>
                            <label className="block text-xs font-bold text-stone-400 uppercase tracking-widest mb-2">Gold Rate Alert Template Name</label>
                            <input type="text" value={config.whatsappGoldRateTemplateName || ''} onChange={e => setConfig({...config, whatsappGoldRateTemplateName: e.target.value})} placeholder="e.g. gold_rate_alert_daily" className="w-full p-2 border border-stone-200 rounded-lg text-sm text-stone-900" />
                        </div>
                        <div>
                            <label className="block text-xs font-bold text-stone-400 uppercase tracking-widest mb-2">Welcome Subscriber Template Name</label>
                            <input type="text" value={config.whatsappWelcomeTemplateName || ''} onChange={e => setConfig({...config, whatsappWelcomeTemplateName: e.target.value})} placeholder="e.g. welcome_subscriber" className="w-full p-2 border border-stone-200 rounded-lg text-sm text-stone-900" />
                        </div>
                    </div>
                    <div>
            ...
            <div className="bg-white p-6 rounded-xl border border-stone-100 shadow-sm">
                <h3 className="font-bold text-stone-700 mb-4 flex items-center gap-2">
                    <svg className="w-5 h-5 text-brand-dark" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                ...
                <p className="text-xs text-stone-500 mb-6 max-w-2xl">
                    Connect your Instagram account to let our AI Stylist learn your brand voice and latest trends from your social media.
                    This requires generating an Instagram Graph API token from the Meta Developer portal.
                </p>
                <div className="space-y-4">
                    <div>
                        <label className="block text-xs font-bold text-stone-400 uppercase tracking-widest mb-2">Instagram Username / Handle</label>
                        <div className="flex bg-stone-50 border border-stone-200 rounded-lg overflow-hidden">
                            <span className="px-3 py-2 text-stone-400 bg-stone-100 border-r border-stone-200">@</span>
                            <input type="text" value={config.instagramHandle || ''} onChange={e => setConfig({...config, instagramHandle: e.target.value})} placeholder="sanghavijewelstudio" className="w-full p-2 text-sm text-stone-900 bg-transparent outline-none" />
                        </div>
                    </div>
                    <div>
                        <label className="block text-xs font-bold text-stone-400 uppercase tracking-widest mb-2">Instagram Access Token (Optional)</label>
                        <div className="relative">
                            <input 
                                type={showPassword ? 'text' : 'password'} 
                                value={config.instagramToken || ''} 
                                onChange={e => setConfig({...config, instagramToken: e.target.value})} 
                                placeholder="Enter Meta Developer Access Token" 
                                className="w-full p-2 border border-stone-200 rounded-lg text-sm text-stone-900 pr-10" 
                            />
                            <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-2 top-1/2 -translate-y-1/2 text-stone-400 hover:text-stone-600">
                                {showPassword ? <EyeOff size={19}/> : <Eye size={19}/>}
